    ...
    async.parallel({
      stats: function (callback) {
        compDbconnMain.models[compDbconnMain.JSON_KEYS.QUEUE_LOG].findAll({
          ...
          attributes: [
            'agent', ['DATE_FORMAT(time,"%d-%m-%Y")', 'period'],
      ...
      for (i in results.stats) {
        values = {};
        for (z in results.stats[i].dataValues) {
  ...
  try {
    ...
    compDbconnMain.models[compDbconnMain.JSON_KEYS.QUEUE_LOG].find({
      ...
      attributes: [
        ...
        ['COUNT(IF(event IN ("COMPLETEAGENT","COMPLETECALLER") AND data2>=' + sla + ', 1, NULL))', 'processed_greater_sla'],
