...
*
* @method setLogger
* @param {object} log The logger object. It must have at least three
*                     methods: _info, warn and error_ as console object.
* @static
*/
...
function checkQueueRecallingStatus(num, cb) {
    try {
        ...
        compAstProxy.doCmd({ command: 'listChannels' }, function (err, results) {
            if (err) { cb(err); }
            else {
...
function analizeQueueRecallingStatus(results, num, cb) {
    try {
        ...
        var ch;
        for (ch in results) {
            if (results[ch].callerNum  === num ||
                results[ch].bridgedNum === num) {
