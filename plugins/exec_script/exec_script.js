...
function setAstProxy(comp) {
    try {
        // check parameter
        if (typeof comp !== 'object') { throw new Error('wrong astProxy object'); }
        compAstProxy = comp;
    } catch (err) {
        logger.error(IDLOG, err.stack);
    }
...
function start() {
    try {
        ...
        if (typeof configJSON !== 'object') {
            logger.warn(IDLOG, 'component is not configured correctly, so it will be ignored');
...
function setAstProxyListeners() {
    try {
        ...
        if (   typeof configJSON.cdr        === 'object'
            ...
            logger.info(IDLOG, 'new listener has been set for "' + compAstProxy.EVT_NEW_CDR + '" event in the astProxy component');
        }
    } catch (err) {
       logger.error(IDLOG, err.stack);
    }
...
function evtNewCdr(data) {
    try {
        ...
        logger.info(IDLOG, 'received event "' + compAstProxy.EVT_NEW_CDR + '" from astProxy component with uniqueid "' + data.uniqueid + '"');
        ...
        logger.info(IDLOG, 'executing script - ' + cmd + ' - with ' + configJSON.cdr.timeout +
                           ' msec timeout after "' + compAstProxy.EVT_NEW_CDR + '" event with uniqueid "' + data.uniqueid + '"');

        childProcess.exec(cmd, { timeout: configJSON.cdr.timeout }, function (error, stdout, stderr) {
            try {
                if (error === null) {
                    logger.info(IDLOG, 'script - ' + cmd + ' - succesfully executed after "' + compAstProxy.EVT_NEW_CDR +
                                       '" event with uniqueid "' + data.uniqueid + '"');
                } else {
                    logger.error(IDLOG, 'executing script - ' + cmd + ' - with ' + configJSON.cdr.timeout + ' msec timeout after "' + compAstProxy.EVT_NEW_CDR +
