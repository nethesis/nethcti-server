...
(function() {
    try {
        ...
        *
        *     ast_proxy.doCmd({ command: 'cfuVmGet', exten: '214' }, function (res) {
        *         // some code
        *     });
        *
        * @class cfuVmGet
            ...
            data: function (data) {
                try {
                    ...
                    if (map[data.actionid]    && data.event === 'DBGetResponse' &&
                        ...
                        if (isCf2Vm) { map[data.actionid](null, { exten: exten, status: 'on',  cf_type: CF_TYPES.unavailable, to: vm }); }
                        else         { map[data.actionid](null, { exten: exten, status: 'off', cf_type: CF_TYPES.unavailable         }); }

                        delete map[data.actionid]; // remove association ActionID-callback

                    } else if (map[data.actionid] && data.response === 'Error') {
                        map[data.actionid](null, { exten: exten, status: 'off', cf_type: CF_TYPES.unavailable });
                        delete map[data.actionid]; // remove association ActionID-callback
                    }
